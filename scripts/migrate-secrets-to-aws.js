...
async function migrate() {
  ...
  // Step 2: Validate all required secrets exist
  logInfo('\nValidating required secrets...');
  const missingSecrets = [];
  
  for (const secret of SECRETS_TO_MIGRATE) {
    if (!envVars[secret.envVar]) {
      missingSecrets.push(secret.envVar);
      logError(`Missing: ${secret.envVar}`);
    } else {
  ...
  if (missingSecrets.length > 0) {
    logError(`\n${missingSecrets.length} required secrets missing from .env.local`);
    logInfo('Add missing secrets and try again');
    process.exit(1);
  }
  ...
  logInfo(`Secrets to create: ${SECRETS_TO_MIGRATE.length}`);
