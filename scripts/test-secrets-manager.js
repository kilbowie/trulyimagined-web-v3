...
async function testCaching(client, secretName) {
  ...
  if (duration2 < duration1 * 0.5) {
    logSuccess(
      `  Caching appears active (${Math.round((1 - duration2 / duration1) * 100)}% faster)`
    );
  } else {
...
async function testSecretsManager() {
  ...
  logHeader('\nCost Estimate');
  log(`Secrets stored: ${successCount} × $0.40/month = $${(successCount * 0.4).toFixed(2)}/month`);
  log('API calls: ~3,000/month × $0.05/10K = $0.02/month');
  log(`Total estimated cost: $${(successCount * 0.4 + 0.02).toFixed(2)}/month`);
