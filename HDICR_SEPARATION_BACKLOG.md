...
| SEP-002 | Inventory and classify all direct DB access in web routes        | 1     | P0       | [x]    |
...
| SEP-043 | Add Row-Level Security policies for tenant isolation             | 4     | P3       | [x]    |
...
- [x] Classification reviewed and agreed before Phase 4 DB split

Implementation note (2026-04-08): Route ownership classification is now complete and reviewed for all 26 inventoried web API routes. Each route includes a `DB-OWNER` annotation, HDICR-owned direct DB accesses are mapped to dedicated SEP tickets, and the inventory baseline is approved for Phase 4 DB split execution.
...
- [x] `DATABASE_URL` removed from TI Vercel environment
...
Implementation note (2026-04-08): Began draining remaining web-tier HDICR reads by switching actor resolution in dashboard, profile, notifications, media routes, dashboard register-identity, and `/api/identity/status` from direct `actors` SQL to existing HDICR representation/identity clients. Removed unused actor registry-ID assignment helpers from `apps/web/src/lib/registry-id.ts` so this module is now agent-only. Added a new HDICR consent-service endpoint `POST /v1/consent/enforcement/check` (license + consent enforcement decision path), and rewired TI `POST /api/v1/consent/check` to proxy that endpoint through `apps/web/src/lib/hdicr/consent-client.ts` instead of importing local `consent-ledger`/`licensing` DB logic. Removed dead `apps/web/src/lib/identity-resolution.ts` (direct `identity_links` SQL) in favor of existing HDICR identity endpoints used by `/api/identity/resolution` and `/api/verification/status`. Migrated Stripe webhook identity-link writes/updates in `apps/web/src/lib/hdicr/stripe-webhook-client.ts` to identity-service APIs and added corresponding identity-service endpoints (`/v1/identity/link/by-provider`, `/v1/identity/links`, `/v1/identity/link/create`, `/v1/identity/link/reactivate`, unlink routes), eliminating TI-local `identity_links` SQL from that webhook path. Removed dead local `apps/web/src/lib/consent-ledger.ts`, `apps/web/src/lib/licensing.ts`, and `apps/web/src/lib/status-list-manager.ts` modules after confirming web runtime routes already depend on HDICR consent/licensing/credentials clients. Added a second hard CI guardrail in `scripts/check-hdicr-owned-table-access.sh` to fail if `apps/web/src` runtime code reintroduces direct SQL against HDICR-owned tables. Extracted TI-owned notification count queries into `apps/web/src/lib/notification-counts.ts`, which removed `apps/web/src/app/api/notifications/counts/route.ts` from the SEP-003 co-import exemption list without changing route behavior. Extracted TI-owned `actor_media` queries into `apps/web/src/lib/actor-media.ts`, which removed the four media API routes from the SEP-003 co-import exemption list while keeping HDICR actor resolution and TI media storage boundaries separate. Centralized actor registration status composition into `apps/web/src/lib/identity-status.ts`, which removed `apps/web/src/app/api/identity/status/route.ts` from the SEP-003 co-import exemption list and de-duplicated status logic with `apps/web/src/app/dashboard/register-identity/page.tsx`. Extracted active roster relationship SQL into `apps/web/src/lib/agent-roster.ts`, which removed `apps/web/src/app/api/agent/roster/route.ts` from the SEP-003 co-import exemption list and completed the route-level exemption burn-down (`scripts/check-hdicr-db-coimport.sh` now has an empty exemptions array). Added TI runtime env decoupling: `apps/web/src/lib/db.ts` now prefers `TI_DATABASE_URL` and only falls back to legacy `DATABASE_URL` with a warning, and `apps/web/.env.example` / debug guidance now document `TI_DATABASE_URL` as the primary TI-only DB connection variable. Added operational cutover support via `docs/SEP042_CUTOVER_RUNBOOK.md` and automated readiness script `scripts/verify-sep042-cutover-readiness.mjs` (wired as `pnpm check:sep042-readiness`) to make pre-cutover verification repeatable before Vercel env removal. Execution evidence (2026-04-08): ran `npx vercel link --yes` (linked project `kilbowies-projects/trulyimagined-web-v3`), `npx vercel env ls production|preview|development` (all returned "No Environment Variables found"), and attempted `npx vercel env rm DATABASE_URL production --yes` / `preview --yes` (both returned "Environment Variable was not found"). TI-owned queries still remain local, so remaining closure work is deployment + smoke validation and log review after this env state.
...
- [x] RLS enabled on all HDICR-owned tables
- [x] Service handlers set `app.current_tenant_id` from JWT claim before queries
- [x] Integration test proves cross-tenant query returns empty result set, not an error
- [x] `trimg_admin` DB role is a superuser exempt from RLS for migrations only

Implementation note (2026-04-08): Added `infra/database/migrations/017_rls_policies.sql` with RLS enablement and tenant-isolation policies for all currently tenant-scoped HDICR tables (`actors`, `consent_log`, `identity_links`, `verifiable_credentials`, `licensing_requests`, `usage_tracking`, `audit_log`, `representation_requests`, `actor_agent_relationships`), plus conditional `ALTER ROLE trimg_admin BYPASSRLS`. Added `queryWithTenant()` to `infra/database/src/client.ts` to execute queries inside a transaction with `SET LOCAL app.current_tenant_id = $1`, and switched `identity-service`, `consent-service`, `licensing-service`, and `representation-service` query paths to this tenant-context execution model. Added integration assertion coverage in `services/identity-service/test/tenant-rls.integration.test.ts` that verifies cross-tenant reads return empty result sets (not errors) when `RUN_RLS_INTEGRATION=1` is set and a real database is available.
